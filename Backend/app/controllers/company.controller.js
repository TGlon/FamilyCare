const { Company } = require("../models/index.model.js");
const createError = require("http-errors");

exports.create = async (req, res, next) => {
  try {
    const document = await Company.create({
      name: req.body.name,
    });
...
exports.findAll = async (req, res, next) => {
  try {
    const documents = await Company.findAll();
...
exports.findOne = async (req, res, next) => {
  try {
    const documents = await Company.findOne({
      where: {
        _id: req.params.id,
      },
    });
...
exports.deleteOne = async (req, res, next) => {
  const { id } = req.params;
  try {
    const document = await Company.destroy({
      where: {
        _id: id,
      },
    });
...
exports.update = async (req, res, next) => {
  ...
  console.log(id);
  try {
    const document = await Company.update(
      {
        name: req.body.name,
      },
      {
