const { Medicine_Type } = require("../models/index.model.js");
const createError = require("http-errors");

exports.create = async (req, res, next) => {
  try {
    const document = await Medicine_Type.create({
      name: req.body.name,
    });
    return res.status(200).json({
      msg: document
        ? "Tạo loại thuốc thành công"
...
exports.findAll = async (req, res, next) => {
  try {
    const documents = await Medicine_Type.findAll();
...
exports.findOne = async (req, res, next) => {
  try {
    const documents = await Medicine_Type.findOne({
      where: {
        _id: req.params.id,
      },
    });
...
exports.deleteOne = async (req, res, next) => {
  const { id } = req.params;
  try {
    const document = await Medicine_Type.destroy({
      where: {
        _id: id,
      },
    });
...
exports.deleteAll = async (req, res, next) => {
    try {
      const deletedCount = await Medicine_Type.destroy({
        where: {},
...
exports.update = async (req, res, next) => {
  ...
  console.log(id);
  try {
    const document = await Medicine_Type.update(
      {
        name: req.body.name,
      },
      {
