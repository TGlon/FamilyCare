    ...
    <div class="d-flex justify-content-between mx-3 mb-3">
      <div class="d-flex justify-content-start">
        ...
        <Search
          ...
          :placeholder="`Tìm Kiếm Theo Tên Và Gia Đình`"
        />
      </div>
...
export default {
  ...
  return data.items.map((value, index) => {
    ...
    const lowerCaseFamilyName = value.familyName.toLowerCase();
    return [lowerCaseName, lowerCaseFamilyName].join("");
  });
});
    ...
    const refresh = async () => {
      try {
        // Lấy tất cả thông tin từ bảng UserFamily
        data.matchingUserFamilies = await http_getAll(User_Family);

        // Tạo một mảng mới để lưu trữ tất cả các FamilyId
        const familyIds = data.matchingUserFamilies.map(
          (item) => item.FamilyId
        );

        // Tạo đối tượng để theo dõi thông tin người dùng theo userId
        const userMap = {};
        ...
        for (const familyId of familyIds) {
          try {
            ...
            const userFamilyDocuments = await http_getAllUserIdByFamilyId(
              User_Family,
              familyId
            );

            // Tạo một mảng để lưu trữ các UserId cho familyId cụ thể
            const userIds = userFamilyDocuments.map(
              (document) => document.UserId
            );

            for (const userId of userIds) {
              try {
                // Kiểm tra xem thông tin người dùng đã được xử lý chưa
                if (!userMap[userId]) {
                  const user = await http_getOne(User, userId);
                  const userRelationships = await http_getAllByUserId(
                    User_Family,
                    userId
                  );
                  const relationships = userRelationships.map(
                    (item) => item.relationship
                  );

                  // Tìm tên gia đình tương ứng với FamilyId
                  const matchingFamily = data.matchingUserFamilies.find(
                    (item) => item.FamilyId === familyId
                  );

                  const familyne = await http_getOne(Family, familyId);
                  const familyName = familyne.name;
                  ...
                  // Thêm thông tin người dùng vào đối tượng userMap để đảm bảo không có giá trị trùng lặp
                  userMap[userId] = userWithRelationship;
                }
              } catch (error) {
                console.error(
        ...
        // Chuyển đối tượng userMap thành mảng để sử dụng
        data.items = Object.values(userMap).map((userWithRelationship) => {
          const { user, relationships, familyName } = userWithRelationship;
          return {
            _id: user._id,
            name: user.name,
            gender: user.gender,
