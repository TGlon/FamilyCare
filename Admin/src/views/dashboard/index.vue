<template>
  <div style="border: 1px solid var(--gray); border-radius: 5px">
    ...
    <div class="lineChart-container" v-if="lineChartVisible">
      <div>
    <label for="userSelect">Chọn Người Dùng Thống Kê:</label>
    <select ref="userSelect" v-model="selectedUserId" class="custom-select">
...
// import 'select2'; // Import JS của Select2
...
export default {
  setup() {
        ...
        window.myPieChart = new Chart(ctx, {
          ...
          options: {
            plugins: {
              title: {
                display: true,
                text: "Biểu Đồ Phân Phối Thuốc Khách Hàng Sử Dụng", // Đặt tên cho biểu đồ tròn ở đây
                position: "bottom",
                fontSize: 50,
              },
              legend: {
    ...
    const users = ref([]); // Assuming you have a list of users
    const selectedUserId = ref(null);
    const healthData = ref([]);
    const loadUsers = async () => {
      // Assuming http_getAll returns an array of User objects
      users.value = await http_getAll(User);
      
      // Kích hoạt Select2 sau khi dữ liệu đã được load
      const userSelectRef = ref("userSelect");
      ...
      if (userSelectRef.value) {
        ...
        // Lắng nghe sự kiện khi giá trị của ô select thay đổi và cập nhật biến selectedUserId
        $(userSelectRef.value).on('change', function () {
    ...
    const loadHealthData = async () => {
      ...
      healthData.value = healthData.value.filter(
        ...
        window.myHealthChart = new Chart(ctx, {
          ...
          options: {
            ...
            plugins: {
              title: {
                display: true,
                text: "Biểu Đồ Thống Kê Sức Khỏe Người Dùng",
                position: "bottom",
              },
            },
          },
    ...
    const toggleLineChart = async () => {
      ...
      if (lineChartVisible.value && selectedUserId.value) {
    ...
    watch(selectedUserId, async () => {
      // Watch for changes in selectedUserId and reload health data and create chart
      if (lineChartVisible.value && selectedUserId.value) {
        await loadHealthData();
        createHealthChart();
      }
...
label {
  display: block; /* Hiển thị label làm một block để có thể đặt các thuộc tính khác */
  margin-bottom: 8px; /* Khoảng cách giữa label và select */
  margin-left: 55px;
  font-weight: bold;
}
/* Định dạng chung cho select */
.custom-select {
  width: 200px; /* Độ rộng của select */
  padding: 8px; /* Khoảng cách giữa nội dung và đường viền của select */
  font-size: 14px; /* Kích thước chữ */
  border: 1px solid #ccc; /* Đường viền của select */
  border-radius: 4px; /* Bo tròn các góc của select */
...
/* Định dạng khi được focus */
.custom-select:focus {
  outline: none; /* Loại bỏ đường viền khi focus */
  box-shadow: 0 0 5px rgba(0, 0, 0, 0.2); /* Hiệu ứng bóng khi focus */
}
