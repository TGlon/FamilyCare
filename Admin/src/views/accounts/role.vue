...
export default {
  ...
  setup() {
    ...
    // Methods
    const create = async () => {
        const createRole = await http_create(Role, data.itemAdd);
        if(createRole){
            alert_success("Thông Báo", "Bạn đã tạo thành công vai trò mới");
            refresh();
        }else {
            alert_error("Lỗi", "Tạo vai trò thất bại")
        }
    };
    ...
    const deleteOne = async (value) => {
        ...
        `Xoá Vai Trò`,
        `Bạn có chắc chắn muốn xoá role này không ?`
      );
      // console.log(isConfirmed);
      if (isConfirmed == true) {
        const result = await http_deleteOne(Role, value);
        alert_success(
          `Xoá Role`,
...
.show-modal {
  ...
  right: 0;
  bottom: 0;
  z-index: 1050; /* Ensure the modal is on top of other content */
}
</style>
